...
* `TF_VAR_enddate`: value for the `enddate` tag to be added to your cloude resources.
* `TF_VAR_project`: value for the `project` tag to be added to your cloude resources.
...
. Edit the `stack.cdp716p.sh` file and update the following variables with your CDP credentials:
+
[source,properties]
----
...
[source,shell]
----
bash -x ./setup.sh <CLOUD_PROVIDER> <SSH_USER> <SSH_PWD> cdp716p <DOCKER_DEVICE>
----
