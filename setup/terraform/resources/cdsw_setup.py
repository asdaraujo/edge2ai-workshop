...
try:
    ...
    r = s.post(CDSW_ALTUS_API + '/models/list-models',
               json={'projectOwnerName': 'admin', 'latestModelDeployment': True, 'latestModelBuild': True})
...
"""
    ...
    # Get CDSW release
    r = s.get(CDSW_API + '/site/stats')
    release_str = [c['value'] for c in r.json() if c['key'] == 'config.release'][0]
    release = [int(v) for v in release_str.split('.')]
